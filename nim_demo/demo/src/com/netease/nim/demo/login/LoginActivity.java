...
import com.netease.nim.demo.mybuild_login.Mybuild_loginActivity;
...
//loginAccountEdit.getEditableText().toString().toLowerCase()
        final String account = ContactHttpClient.getInstance().user_only_accid;
        final String token = ContactHttpClient.getInstance().user_only_token;
        // 登录
        Log.e("用户account",account);
        Log.e("用户token",token);
        loginRequest = NimUIKit.login(new LoginInfo(account, token), new RequestCallback<LoginInfo>() {
            @Override
            public void onSuccess(LoginInfo param) {
                Log.d("成功","+new LoginInfo().getAppKey()+");
                ...
                Intent inte = new Intent(LoginActivity.this,Mybuild_loginActivity.class);
                startActivity(inte);
//               MainActivity.start(LoginActivity.this, null);
    ...
    private void register() {
        if (!registerMode || !registerPanelInited) {//是否开启了注册模式
            return;
        }

        if (!checkRegisterContentValid()) {//注册验证
            return;
        }
